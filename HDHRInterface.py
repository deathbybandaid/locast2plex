...
class PlexHttpHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if (self.path == '/') or (self.path == '/device.xml'):
            ...
            loading_file = "/home/loading.png"
            ffmpeg_proc = subprocess.Popen([self.ffpmeg_path, "-i", loading_file, "-codec", "copy", "-f", "mpegts", "pipe:1"], stdout=subprocess.PIPE)
            videoData = ffmpeg_proc.stdout.read()
            self.wfile.write(videoData)

            ffmpeg_proc = subprocess.Popen([self.ffpmeg_path, "-i", channelUri, "-codec", "copy", "-f", "mpegts", "pipe:1"], stdout=subprocess.PIPE)
            ...
            dmafile = str(self.script_dir + "/data/cache/" + str(self.location["DMA"]) + ".xml")
...
def hdhrinterface_start(config, locast, station_list, script_dir, location):
    ...
    for i in range(int(config.config["locast2plex"]["concurrent_listeners"])):
        PlexHttpServer(serverSocket, config, templates, station_list, locast, script_dir, location)
