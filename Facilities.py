...
def mainfacilities(script_dir, config):
    get_facilities(script_dir, config)
...
def get_facilities(script_dir, config):

    base_data_dir = pathlib.Path(__file__).parent.joinpath('data')
    base_cache_dir = pathlib.Path(base_data_dir).joinpath('cache')
    if not base_cache_dir.is_dir():
        base_cache_dir.mkdir()
    fcc_cache_dir = pathlib.Path(base_cache_dir).joinpath("facilities")
